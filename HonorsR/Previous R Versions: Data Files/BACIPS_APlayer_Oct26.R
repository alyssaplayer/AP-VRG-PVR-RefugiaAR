...
ProgressiveChangeBACIPS <- function(control, impact, time.true, time.model) 
{
  ...
  png(file = paste(i, "_plot_PVR.png", sep = ""), width=14, height=7, unit="in", res = 200)
  par(mfrow=c(1,2))
  plot(delta~time.true, type="n")
  time.model.of.impact=max(which(time.model==0))
  abline(v = 2020, col = "blue", lty = 5)
  rect(2013, min(delta)-100, 2016, max(delta)+100, col = "orange", border = NULL)
...
### !!! SHOULD WE COMPARE MIDDLE ZONES ONLY TO ANSWER THIS QUESTION?!!! DO WE
### REDUCE THE NUMBER OF SITES?
### HOW DO WE PAIR THESE SITES?
...
ProgressiveChangeBACIPS <- function(control, impact, time.true, time.model) 
{
  ...
  # Plot delta against time.true
  # dev.new(width=10, height=5)
  png(file = paste(i, "_plot_MPA.png", sep = ""))
  par(mfrow=c(1,2))
  plot(delta~time.true, type="n")
  time.model.of.impact=max(which(time.model==0))

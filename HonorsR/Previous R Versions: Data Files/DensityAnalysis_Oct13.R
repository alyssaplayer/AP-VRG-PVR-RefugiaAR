...
# Add year and density per 100m2, and averages 
data_PV <- data_PV %>%
  mutate(SampleDate = dmy(SampleDate),
                Year = year(SampleDate),
                Density_100m2 = 100 * Density_m2) %>%
  ungroup() %>% droplevels() %>%
  summarize(Avg_Density = mean(Density_100m2, na.rm = TRUE)) %>%
  group_by(Site, Year) #%>%
  summarize(Avg_Density = mean(Density_100m2, na.rm = TRUE))
#Looks for grouped data of the nesting variables and if there is N/A, it inputs 0 for both density variables
...
ggplot(data_PV, aes(x = Year, y = Avg_Density, color = Category)) +
...
ggplot(pisgig, aes(x = Year, y = Density_100m2, color = Category)) +
  ...
  facet_wrap(~ Species, scales = "free_y")+
  annotate("rect", xmin = 2013, xmax = 2016, ymin = 0, ymax = Inf,
           alpha = .3,fill = "chocolate1")

avg_pisgig <- data_PV %>%
    group_by(Year)%>%
    mutate(avg_density_year = mean(Density_100m2)) %>%
    filter(Species=="Pisaster giganteus", Year >= 2008)
    ungroup()
    
ggplot(avg_pisgig, aes(x = Year, y = avg_density_year, color = Category)) +
  geom_line(position = position_dodge(width = 0.5)) +
  geom_vline(xintercept = 2020, linetype = "dashed", color = "palegreen4", alpha = 0.8) + #Adds dashed line indicating completed construction of PVR
  theme_gray()+
  theme(axis.line = element_line(colour = "black"),
